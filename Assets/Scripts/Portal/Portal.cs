...
public class Portal : MonoBehaviour {
    ...
    // Рендеринг портала без рекурсии
    public void Render()
    {
        // Проверяем, виден ли связанный портал для камеры игрока.
        if (!CameraUtility.VisibleFromCamera(linkedPortal.screen, playerCam))
            return;

        // Создаем или обновляем RenderTexture для вида портала.
        CreateViewTexture();

        // Вычисляем новую матрицу трансформации для портальной камеры.
        // Здесь мы один раз преобразуем матрицу камеры игрока в систему координат связанного портала.
        Matrix4x4 newMatrix = transform.localToWorldMatrix * linkedPortal.transform.worldToLocalMatrix * playerCam.transform.localToWorldMatrix;
        Vector3 newPosition = newMatrix.GetColumn(3);
        Quaternion newRotation = newMatrix.rotation;

        // Настраиваем портальную камеру: задаем позицию, поворот и проекционную матрицу.
        portalCam.projectionMatrix = playerCam.projectionMatrix;
        portalCam.transform.SetPositionAndRotation(newPosition, newRotation);

        // Скрываем экран портала, чтобы камера могла «смотреть сквозь него».
        screen.shadowCastingMode = UnityEngine.Rendering.ShadowCastingMode.ShadowsOnly;
        linkedPortal.screen.material.SetInt("displayMask", 0);

        // При необходимости устанавливаем ближнюю плоскость отсечения и другие настройки клиппинга.
        SetNearClipPlane();
        HandleClipping();   


        // Восстанавливаем настройки: включаем отображение связанного портала и возвращаем режим теневого кастинга.
